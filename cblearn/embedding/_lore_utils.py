...
def _logistic_triplet_loss_numpy(X, triplets, margin, eps=1e-3):
    ...
    Uses the smooth distance ``d(x, y) = sqrt(||x - y||² + ε²)`` so that the
    ...
    # Smooth Euclidean distance: sqrt(||.||² + ε²)
...
def _estimate_lipschitz_numpy(X, triplets, margin, num_iters=100, tol=1e-6, fd_eps=1e-5):
    ...
    for _ in range(num_iters):
        # Finite-difference Hessian-vector product: (∇f(X + ε·V) − ∇f(X)) / ε
...
def _lore_train_scipy(init, triplets, lamb, p, margin, mu,
    ...
    Args:
        ...
        lamb: Regularisation strength λ.
    ...
    if mu == "default":
        ...
        if verbose:
            print(f"[scipy] Estimated Lipschitz constant (μ): {mu:.4f}")
    else:
        mu = float(mu)
...
def _logistic_triplet_loss_torch(embedding, triplets_t, margin, eps=1e-3):
    ...
    Uses the smooth distance ``sqrt(||x - y||² + ε²)`` matching the scipy
...
def _lore_train_torch(init, triplets, lamb, p, margin, mu,
    ...
    if mu == "default":
        ...
        if verbose:
            print(f"Estimated Lipschitz constant (μ): {mu_val:.4f}")
    else:
        mu_val = float(mu)
